...
import { isRegExp, mapValues } from "./utils";
...
function toFilter(filter) {
    ...
    throw new TypeError(
        "[ComposeMiddleware] Middleware filter must be RegExp, String or Function."
    );
}
...
export class ComposeMiddleware {
    static toStandardMiddleware(_middleware = {}) {
        if (typeof _middleware === "function") {
            ...
            Object.keys(_middleware).forEach(key => {
                if (!KEYS.includes(key)) {
                    throw new Error(
                        `[ComposeMiddleware] Middleware key must one of "${KEYS.join(
                            " ,"
                        )}"`
                    );
                }
        ...
        throw new TypeError(
            "[ComposeMiddleware] Middleware must be a function or object but get " +
                _middleware
        );
    }
        ...
        return this._before
            ...
            .then(args => {
                if (!Array.isArray(args)) {
                    throw new Error(
                        "[ComposeMiddleware] Pre middleware must return arguments"
                    );
                }
